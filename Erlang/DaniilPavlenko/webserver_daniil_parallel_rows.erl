-module(webserver_daniil_parallel_rows).
...
process_request(Bin) -> 
    ...
    matrix_to_binary_float32(multiply_matrixes_tr_parallel(A,TB)).
...
multiply_matrixes_tr_parallel(A,TB) -> 
    S = self(),
    Pids = lists:map(
        fun(RowA) ->
            spawn(fun()-> calc_row(S,RowA,TB) end)
        end, A),
    gather_rows(Pids).

calc_row(Parent,Ra,TB) -> 
    Parent ! {self(), [lists:sum(lists:zipwith(fun(X,Y)->X*Y end,Rb,Ra)) || Rb <- TB]}.

gather_rows([]) -> [];
gather_rows([Pid|T]) ->
    receive
        {Pid,Row} -> [Row|gather_rows(T)]
    end.